...
dep_func <- function(dat, window, base_var, time_var, covar_coef, m_haz, num_t, tcov_n){ # should return a binary variable for M, and continuous for all other tvar
  ...
  if(all(dim(dat) != c(dim, dim))) stop("'dat' must have dimensions \n and \y")
  if(length(i) != 1) stop("'window' must be length 1")
  if(length(base_var) != some_length) stop("")
  if(length(time_var) != some_length) stop("")
  if(all(dim(covar_coef$baseline) != c(dim, dim))) stop("")
  if(all(dim(covar_coef$varying) != c(dim, dim))) stop("")
  if(length(m_haz) != some_length) stop("")
  ...
  # dat$time (integer, containing window), dat$[all names in base_var], dat$treat, dat$[names in time_var]
  # dat$M, dat$Mtime
