...
simswitchloop <- function(add_tvar, b_allowance, b_haz, b_mag, b_scale, b_shape, bcov, beta.mat,
  ...
  trt <- rep(0, length(ids)) # create treatment covariate
  ...
  if(missing(bcov)){ # if a baseline covariate matrix is not specified, generate a random one
    ...
    diag(covariance) <- 0.4
    # covariance[upper.tri(covariance)] <- t(covariance)[upper.tri(covariance)] # force symmetric
  ...
  scale_bcov <- scale(x = bcov, center = TRUE, scale = TRUE) # save scaled bcov vectors for later generation of the starting values for tcovs
  ...
  fulldat_cont <- fulldat # set a counterfactual group, where there is no time-dependent confounding


  # TODO don't need the entire covar_coef vector
  ...
  # TODO don't need the entire dep_func() function
  ...
  #       retval <- (as.matrix(dat[dat$time == window, names(dat) %in% base_var]) %*% covar_coef$baseline) + MASS::mvrnorm(n = sum(dat$time == window), rep.int(0,num_tvar), diag(0.1,nrow = num_tvar))
  ...
  #       retval <- (as.matrix(dat[dat$time == (window-1), names(dat) %in% c("treat", time_var)]) %*% covar_coef$varying) + MASS::mvrnorm(n = sum(dat$time==(window-1)), rep.int(0, num_tvar), diag(0.015, nrow = num_tvar)) # TODO sum(dat$time==(window-1)) can probably just be n
  ...
  #                                                                    as.matrix(dat[dat$time == window-1, names(dat) %in% c("treat", tcov_names)]) %*% covar_coef$varying[,1])) ))[retval[,1] == 0] # randomly assign the treat variable with probability
  #       retval <- cbind(retval, rep.int(0, length(retval[,1])))
  #       retval[,num_tvar + 1] <- ifelse(retval[,1] == 0, 0,  # add a column representing Mtime, and calculate Mtime.
  ...
  # a dataframe with the means of all (random) arguments to the bb() function. Each row is a covariate, each column a different argument.
  # TODO tvar_mean tvar_var should probably be FIXED outside of for loop. then, covariates have true generating functions. more stable estimaion.
  if(missing(baseline_to_tvar_start)){
    baseline_to_tvar_start <- matrix(rnorm(n = num_bvar*num_tvar, mean = 0, sd = 1), nrow = num_bvar, ncol = num_tvar)
  }
  if(missing(baseline_to_tvar_end)){
    baseline_to_tvar_end <- matrix(rnorm(n = num_bvar*num_tvar, mean = 0, sd = 1), nrow = num_bvar, ncol = num_tvar)
  }
  if(missing(baseline_to_tvar_var)){
    baseline_to_tvar_var <- matrix(rnorm(n = num_bvar*num_tvar, mean = 0, sd = 0.1), nrow = num_bvar, ncol = num_tvar)
  }
  if(missing(baseline_to_tvar_tr_fx)){
    baseline_to_tvar_tr_fx <- matrix(rnorm(n = num_bvar*num_tvar, mean = 1, sd = 1), nrow = num_bvar, ncol = num_tvar)
  }
  if(missing(tvar_var)){
    tvar_var <- data.frame(s = rexp(num_tvar, 3),
                           e = rexp(num_tvar, 3),
                           tr = rexp(num_tvar, 3))
  }
  if(missing(tvar_lim)){
  ...
  for(r in 1:reps){
    ...
    # TODO for paralellization purposes, this should be moved up and out of the giant for loop. that way, the remaining loop can be paralellized entirely
    ...
    while(rerun){ # iteratively update switching hazard function until we get the right proportion. The first term in this check is the number of pts whose final treatment indicator
      ...
      # TODO generate covariates here INSTEAD of within the subsequent 1:stime loop. Then calculate prob of treatment within the loop,
      # and if treatment begins, begin calling treat() within the loop on all times after given time.
      # Generate brownian-bridge covariates here
      # inputs: stime
      # num_tvar
      # mean var for each tvar start
      # gain and var for each tvar end
      # e.g., 3 tvars
      # add an argument called tvar_mean and tvar_var

      # make a list of bridge objects, which is each a time-varying covariate, until the iteration is completed and all treatments are applied
      bridge_list <- list()

      # generate NON-TREATED covars. I.e., "baseline" covars, or covariate values that WOULD have been observed if treatment was never applied.
      for(i in 1:num_tvar){ # for each tvar, call bb()

        # TODO all s, e, v, ti, tr must at least be functions of baseline.
        s <- rnorm(n = n, mean = scale_bcov %*% baseline_to_tvar_start[,i], sd = tvar_var$s[i]) # generate start values. conditional Expect of start values is function of baseline, but variance of start values is fixed
        e <- rnorm(n = n, mean = scale_bcov %*% baseline_to_tvar_end[,i], sd = tvar_var$e[i]) # generate end values. conditional Expect of end values is function of baseline, but variance of end values is fixed
        v <- rexp(n = n, rate = exp(scale_bcov %*% baseline_to_tvar_var[,i])) # generate variance values. conditional Expect of start values is function of baseline, and is exp()ed so always positive
        ti <- stime
        tr <- rnorm(n = n, mean = scale_bcov %*% baseline_to_tvar_tr_fx[,i], sd = tvar_var$tr[i]) # generate additive treatment values. conditional Expect of treatment values is function of baseline, but variance of treatment values is fixed
        if(is.null(tvar_lim)){
          # there is no tvar_lim
          u <- NULL # get from function call. if missing, NULL
          l <- NULL # get from function call. if missing, NULL
        } else{
          # there is a tvar_lim
          u <- tvar_lim$u[i] # get from function call. if missing, NULL
          l <- tvar_lim$l[i] # get from function call. if missing, NULL
        }
        if(is.null(tvar_tran)){
          tran <- NULL # get from function call. if missing, NULL
          if(i == 1) tran <- binarize # if we're generating the first covariate, transform to a probability scale
        } else{
          tran <- tvar_tran[i] # get from function call. if missing, NULL
        }

        # create a brownian bridge of appropriate dimension and with appropriate start/end
        bridge_list[[i]] <- bb(start = s, end = e, var = v, time = ti, trt = tr, uppr = u, lwr = l, trnsfrm = tran)

      }

      # make a copy of bridge_list[] which will be treated only according to protocol
      bridge_list_cont <- bridge_list

      # add TREATMENT to non-treated covars. Iterate by time and, for each time, update covars if treated and update treatment given covars.
      for(i in 1:stime){
        ...
        # TODO this has what shape? and what shape does treat() need to accept in the brij argument? treat() may need to loop over
        for(j in 1:length(bridge_list)){
          bridge_list[[j]] <- treat(brij = bridge_list[[j]], trt_time = i, trt_vec = fulldat$treat[fulldat$time == i])
        }

        # slot bridge_list at time i back into fulldat. TODO this is not strictly necessary and likely much slower than it could be. We could simply use data directly from bridge_list to update treatments, and then slot the whole
        # data list back in when all covars are updated. This solution is only neccessary so as not to have to change the current treatment updator, which calls fulldat a stupid number of times.
        for(j in 1:length(bridge_list)){
          if(j == 1){ # if it's the first covar, i.e., if covar == M
            # binarize the 1st covar, M, and update all subsequent M values
            f <- apply(bridge_list[[j]]@data, 2, function(x) rbinom(stime,1, bound(x, 0, 1))) # assign M-event (1) with cutoff given by each element of matrix
            f <- as.data.frame(f) # make holder variable of class dataframe
            f[f == 0] <- NA # replace 0s as NAs
            f <- fill(f, names(f)) # fill() downwards (replace NAs after 1s with 1s)
            f[is.na(f)] <- 0 # revert remaining NAs back to 0s
            f <- as.matrix(f) # cast back as matrix and slot into bridge_list at appropriate location
            fulldat[fulldat$time == i, names(fulldat) %in% tcov_names[j]] <- f[j,]
          }else{
            fulldat[fulldat$time == i, names(fulldat) %in% tcov_names[j]] <- bridge_list[[j]]@data[i,]
          }
        }
        # slot Mtime at time i back into fulldat
        if(i == 1){
          fulldat[fulldat$time == i, names(fulldat) %in% "Mtime"] <- ifelse(f[1,] == 1, 1, 0) # if M has already occured, start Mtime
        }else{
          fulldat[fulldat$time == i, names(fulldat) %in% "Mtime"] <- ifelse(fulldat$Mtime[fulldat$time == i-1] != 0, fulldat$Mtime[fulldat$time == i-1] + 1,
                                                                            ifelse(fulldat$M[fulldat$time == i] == 1, 1, 0)
                                                                            )
        }
        ...
        # TODO TODO adjust fulldat$treat based on bridge_list elements
        if(m_hard){
      ...
      # TODO here, reincorporate bridge_list[] into the full dataframe, fulldat

      # Generate fulldat_cont (the control dataset) by blocking switching
      fulldat_cont$treat <- fulldat_cont$arm
      # TODO apply treat(brij) for each i in stime, and then reincorporate bridge_list_cont into fulldat_cont
      for(i in 1:stime){
        # set covars
      ...
      # TODO m_haz needs to somehow adjust the transformation from a probability-brownian bridge to a 0-1 vector.
  ...
  return(list(unbiased = unbiased,
              ...
              pp_plot = pp_plot,
              ipcw = msm_hr,
              ipcw_plot = if(!ipcw_exclude) ipcw_plot,
              rpsft = rpsft,
